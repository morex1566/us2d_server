...
- 사용자가 작성한 프롬프트를 바탕으로 마크다운으로 작성된 표준 개발 절차(SDP)로 작성합니다. SDP를 `implementations/`(없다면 작업중인 메인 에이전트가 동작하는 폴더에서 생성) 폴더에서 작업합니다.
...
- 이 계층을 작업하기 전에는 필수적으로 사용자에게 작업 여부(Proceed/Dismiss)를 질문합니다.
...
에러는 학습의 기회입니다. 무언가 고장 나면:
1. 수정합니다.
2. 도구를 업데이트합니다.
3. 도구를 테스트하고 작동을 확인합니다.
4. 새로운 흐름을 포함하도록 지시서를 업데이트합니다.
...
- 레퍼런스를 참조합니다. : `C:\Users\morex\Documents\us2d_server\agents\convention.cpp`
